...
public final class ElevationChartUtils
{
  ...
  public static void configureYAxisBounds(@NonNull LineChart chart, float minAltitude, float maxAltitude)
  {
    ...
    if (range < 1f)
    {
      ...
      float lower = (float) (Math.floor((minAltitude - altitudeStep) / altitudeStep) * altitudeStep);
      float upper = (float) (Math.ceil((maxAltitude + altitudeStep) / altitudeStep) * altitudeStep);
      int stepCount = Math.round((upper - lower) / altitudeStep);
      YAxis y = chart.getAxisLeft();
